...
from PyQt5.QtCore import Qt, QDateTime, QSettings, QTimer
...
class AplikacjaSledzeniaWydatkow(QMainWindow):
    ...
    def inicjalizacja_interfejsu(self):
        ...
        palette.setColor(QPalette.WindowText, QColor(255, 255, 255))  # Kolor tekstu
        palette.setColor(QPalette.Button, QColor(0, 0, 0))
        palette.setColor(QPalette.ButtonText, QColor(255, 255, 255))  # Kolor tekstu na przyciskach
        palette.setColor(QPalette.Base, QColor(0, 0, 0))
        palette.setColor(QPalette.Text, QColor(255, 255, 255))
        palette.setColor(QPalette.Window, QColor(0, 0, 0))

        # Ustawienia kolorów dla pól do wprowadzania danych
        palette.setColor(QPalette.WindowText, QColor(255, 255, 255))  # Kolor tekstu
        palette.setColor(QPalette.Button, QColor(0, 0, 0))           # Kolor tła pola do wprowadzania danych
        palette.setColor(QPalette.ButtonText, QColor(255, 255, 255))  # Kolor tekstu w polu do wprowadzania danych
        palette.setColor(QPalette.Text, QColor(255, 255, 255))       # Kolor tekstu w polu do wprowadzania danych
        self.setPalette(palette)

        # Ustawienie koloru przycisku "Dodaj Transakcję" na czarny
        przycisk_palette = self.przycisk_dodaj.palette()
        przycisk_palette.setColor(QPalette.ButtonText, QColor(0, 0, 0))
        przycisk_palette.setColor(QPalette.Button, QColor(255, 255, 255))
        self.przycisk_dodaj.setPalette(przycisk_palette)
