...
class AplikacjaSledzeniaWydatkow(QMainWindow):
    ...
    def inicjalizacja_interfejsu(self):
        ...
        układ_przyciski = QVBoxLayout()
        układ_przyciski.addWidget(self.etykieta_kwota)
        układ_przyciski.addWidget(self.pole_kwota)
        układ_przyciski.addWidget(self.etykieta_kategoria)
        układ_przyciski.addWidget(self.pole_kategoria)
        układ_przyciski.addWidget(self.etykieta_przedmiot)
        układ_przyciski.addWidget(self.pole_przedmiot)
        układ_przyciski.addWidget(self.etykieta_data)
        układ_przyciski.addWidget(self.pole_data)
        układ_przyciski.addWidget(self.przycisk_dodaj_wydatek)
        układ_przyciski.addWidget(self.przycisk_dodaj_zarobek)

        # Zmniejszenie rozmiaru okienka
        self.setGeometry(100, 100, 800, 600)

        # Utworzenie układu dla transakcji i zarobków
        układ_transakcje_zarobki = QHBoxLayout()
        układ_transakcje_zarobki.addWidget(self.tekst_transakcje_wydatki)
        układ_transakcje_zarobki.addWidget(self.tekst_transakcje_zarobki)

        # Utworzenie głównego układu
        układ_główny = QVBoxLayout()
        układ_główny.addLayout(układ_przyciski)
        układ_główny.addLayout(układ_transakcje_zarobki)

        # Dodanie tekstu statystyk do układu głównego
        układ_główny.addWidget(self.tekst_statystyki_kategorii)  # Dodaj tę linię

        # Utworzenie widgetu centralnego i przypisanie układu
        centralny_widget = QWidget(self)
        centralny_widget.setLayout(układ_główny)
        ...
        self.tracker_zarobki = TrackerZarobków()
    ...
    def dodaj_transakcje(self, tracker, tekst_transakcje, is_wydatek):
        ...
        self.aktualizuj_wyswietlanie_transakcji(tekst_transakcje, tracker)  # Poprawiony fragment
    ...
    def przejdz_do_kolejnego_pola(self):
        ...
        # Ustaw fokus na kolejnym widżecie, jeśli istnieje
...
class TrackerZarobków(TrackerWydatkow):
    pass
